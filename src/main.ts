...
window.onload = () => {
    ...
    for (let day = 1; day <= 7; day++) {
        let input = <HTMLInputElement>$('day' + day)
        input.onchange = doSubmit;
    }
}

let payBlocksToString = (blocks: PayBlock[]): string =>
    blocks.map(block => `${block.hours} hours @ ${block.rate}x: ${block.reason}`).join(' | ');

function readInputs(): number[] {
    let hours: number[] = [0, 0, 0, 0, 0, 0, 0]
    for (let day = 1; day <= 7; day++) {
        let input = <HTMLInputElement>$('day' + day)
        if (input.value)
...
function renderTable(data: PayBlock[][]) {
    const labels = ["Thursday", "Friday", "Saturday", "Sunday", "Monday", "Tuesday", "Wednesday"]
    const renderHeader = (table: HTMLTableElement) => {
    ...
    const renderLine = (table: HTMLTableElement, label: string, data: PayBlock[]) => {
